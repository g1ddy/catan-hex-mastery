...
After Graphviz succeeds, the command inspects the generated SVG itself—not merely the DOT—and prints its point dimensions, aspect ratio, and rendered node, edge, and cluster counts beside the same measurements from the reference. It rejects an SVG that loses module nodes or recursive clusters and writes the candidate atomically. A candidate is not accepted or described as layout-compatible until this output and a visual side-by-side inspection have been completed.
