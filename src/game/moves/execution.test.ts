...
describe('Catan Lifecycle Rules Seam', () => {
  ...
  describe('executeCatanMove execution & lifecycle transitions across move families', () => {
    ...
    it('executes dismissRobber during ROBBER phase', () => {
      const { hexes, ports } = generateBoard();
      const hexList = Object.values(hexes);
      const hId = hexList[1].id;

      const G = createMockGameState({
        board: { hexes, ports, vertices: {}, edges: {} },
        players: { '0': createTestPlayer('0') },
        robberLocation: hexList[0].id,
      });
      const ctx: GameContext = {
        currentPlayer: '0',
        turn: 1,
      ...
      const result = executeCatanMove(G, ctx, { move: 'dismissRobber', args: [hId, undefined] }, mockRng);

      expect(result.G.robberLocation).toBe(hId);
      expect(result.ctx.stagesByPlayer).toEqual({ '0': STAGES.ACTING });
    });

    it('executes buildSettlement during acting phase', () => {
      const { hexes, ports } = generateBoard();
      const hexList = Object.values(hexes);
      const vId = getVerticesForHex(hexList[0].coords)[0];
      const eId = getEdgesForHex(hexList[0].coords)[0];
      ...
      const G = createMockGameState({
        board: { hexes, ports, vertices: {}, edges: { [eId]: { owner: '0' } } },
        players: {
          '0': createTestPlayer('0', {
      ...
      const result = executeCatanMove(G, ctx, { move: 'buildSettlement', args: [vId] }, mockRng);

      expect(result.G.players['0'].settlements).toContain(vId);
      expect(result.G.players['0'].resources.wood).toBe(0);
      expect(result.G.players['0'].resources.brick).toBe(0);
      expect(result.G.players['0'].resources.sheep).toBe(0);
      expect(result.G.players['0'].resources.wheat).toBe(0);
    });

    it('throws error for unsupported buyDevCard', () => {
      ...
      expect(() => {
        executeCatanMove(G, ctx, { move: 'buyDevCard', args: [] }, mockRng);
      }).toThrow('buyDevCard is not supported yet');
    });
