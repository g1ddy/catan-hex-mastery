...
*   **Layer Boundaries**: Engine logic in MOVES (`src/game/moves/`) must never import from BOTS (`src/bots/`). UI components (`src/features/`) consume Catan view contracts; logic must not bleed into component render trees. Feature code and protected game-domain layers (`src/game/{core,rules,geometry}`, `src/features`, `src/bots`) must consume Catan-owned contracts or adapter gateways, while `Game.ts` and `GameClient.tsx` remain explicit `boardgame.io` composition boundaries during #470 (see [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md)).
