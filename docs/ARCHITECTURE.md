...
The overview is generated by Maritime's `architecture-overview` profile from the same canonical `.maritime/dependency-graph.json` evidence used by the detailed graph. It aggregates file-level relationships into source-root-relative folder nodes so major architectural areas and their coupling are easier to scan. It is a derived presentation, not separate analysis evidence.

### Detailed dependency graph

![Dependency Graph](images/dependency-graph.svg)

The detailed dependency SVG is generated by Maritime's `compact-architecture` profile from `.maritime/dependency-graph.json`; it is not hand-maintained. It preserves individual files while omitting external-package noise and applying compact architecture styling. The Maritime evidence profile reuses Catan's architecture policy while omitting test/spec helpers so these presentations focus on production coupling. Full enforcement still uses `config/dependency-cruiser.cjs` against the complete `src/` tree.
...
*   **Configuration**: The authoritative rules are defined in `config/dependency-cruiser.cjs`; `config/dependency-cruiser.maritime.cjs` inherits those rules only to scope committed evidence to production modules.
