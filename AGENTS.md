...
*   **Command Pattern:** Use this command to run component tests correctly without modifying the global config:
    ```bash
    npx jest tests/my_component.test.tsx --config='{"preset": "ts-jest", "testEnvironment": "jsdom", "transform": {"^.+\\.tsx?$": ["ts-jest", {"tsconfig": {"jsx": "react", "esModuleInterop": true}}]}}'
    ```
