...
The committed dependency SVG is generated by Maritime from `.maritime/dependency-graph.json`; it is not hand-maintained. The Maritime evidence profile reuses Catan's architecture policy while omitting test/spec helpers so this presentation focuses on production coupling. Full enforcement still uses `config/dependency-cruiser.cjs` against the complete `src/` tree.
